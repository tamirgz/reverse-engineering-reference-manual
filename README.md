...
* __Using Functions from Dynamically Linked Libraries to Detect Debugger's Presence__ 
  ...
  * A less obvious function is puts(), which takes in a string and prints it to the console. Under GDB, if you call the binary with argv[0] (name of the current invoked program) it will print out the binary's absolute path even if you invoked the binary from a relative path
