...
* __Using Functions from Dynamically Linked Libraries to Detect Debugger's Presence__ 
  ...
  * __readlink (Linux)__: calling readlink on "/proc/ppid/exe" will return a string containing the location of the debugger if one is attached. You can find ppid by checking the dynamic file /proc/pid/self. And to find the pid of the process, use the ps command
...
* __cat /proc/pid/status (Linux)__: a dynamic file that contains information on a running process, such as whether or not the process is being traced. If the field tracerPid is 0, the process is not being traced
#
